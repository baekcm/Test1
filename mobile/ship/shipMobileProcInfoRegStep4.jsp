...
<tiles:insertDefinition name="app.mobile">
	...
	<div class="wp-wrap">
		<form method="post" id="js-ship-mobile-proc-reg-form">
			...
			<input type="hidden" id="js-ship-mobile-proc-reg-amine-yn" name="js-ship-mobile-proc-reg-amine-yn">
			<input type="hidden" id="js-ship-mobile-proc-reg-sweat-icing-yn" name="js-ship-mobile-proc-reg-sweat-icing-yn">
			<!-- header -->
			...
			<section class="wp-container wp-pb50 wp-bg-f">
				...
				<h1 class="wp-section-title wp-border-none"><spring:message code="TODO.KEY" text="4단계.  날씨 정보"/></h1>
						...
						<tbody>
							<tr>
								<td>
									...
									<select id="js-ship-mobile-proc-reg-weather-condition" name="js-ship-mobile-proc-reg-weather-condition">
									</select>
									</div>
								</td>
								<td><input type="text" id="js-ship-mobile-proc-reg-dt" name="js-ship-mobile-proc-reg-dt" title="<spring:message code='TODO.KEY' text='내용입력'/>" placeholder="<spring:message code="TODO.KEY" text="DT"/>"></td>
							</tr>
							<tr>
								<td><input type="text" id="js-ship-mobile-proc-reg-wt" name="js-ship-mobile-proc-reg-wt"  title="<spring:message code='TODO.KEY' text='내용입력'/>" placeholder="<spring:message code="TODO.KEY" text="WT"/>"></td>
								<td><input type="text" id="js-ship-mobile-proc-reg-st" name="js-ship-mobile-proc-reg-st"  title="<spring:message code='TODO.KEY' text='내용입력'/>" placeholder="<spring:message code="TODO.KEY" text="ST"/>"></td>
							</tr>
							<tr>
								<td><input type="text" id="js-ship-mobile-proc-reg-dp-display" title="<spring:message code='TODO.KEY' text='내용입력'/>" placeholder="DP" disabled=""></td>
								<td><input type="text" id="js-ship-mobile-proc-reg-rh-display" title="<spring:message code='TODO.KEY' text='내용입력'/>" placeholder="RH" disabled=""></td>
							</tr>
							<tr>
								<td colspan="2">
									<input type="text" id="js-ship-mobile-proc-reg-insp-time" name="js-ship-mobile-proc-reg-insp-time" title="<spring:message code='TODO.KEY' text='내용입력'/>" placeholder="<spring:message code='TODO.KEY' text='측정시간'/>">
								</td>
							</tr>
							<tr>
								<th><spring:message code="TODO.KEY" text="Amine migration"/></th>
								<td class="wp-toggle-area"><div id="js-ship-mobile-proc-reg-amine-div" class="wp-toggle toggle-light"></div></td>
							</tr>
							<tr>
								<th><spring:message code="TODO.KEY" text="Sweat or Icing"/></th>
								<td class="wp-toggle-area"><div id="js-ship-mobile-proc-reg-sweat-icing-div" class="wp-toggle toggle-light"></div></td>
							</tr>
						</tbody>
					</table>
					</div>
	...
	<tiles:putAttribute name="page-level-js">
		...
		<script type="text/javascript">
			...
			$(document).ready(function(){
				...
				//DT, WT change 이벤트
				$('#js-ship-mobile-proc-reg-dt, #js-ship-mobile-proc-reg-wt').off().on('change', function(){
					//DP, RH 조회
					retrieveHumidityInfo.request();
				});
				
				//소수점만 받기
				onlyDecimal($('#js-ship-mobile-proc-reg-st'), 5, 1);
				
				//소수점만 받기, 소수점 첫째자리는 5 혹은 0으로 제한
				var prev = '';
				var regExp = new RegExp('^(\\d{0,5}([.]\\d{0,1})?)?$');
				
				$('#js-ship-mobile-proc-reg-dt, #js-ship-mobile-proc-reg-wt').on('keypress', function(e) {
				...
				$('#js-ship-mobile-proc-reg-dt, #js-ship-mobile-proc-reg-wt').on('keyup', function(e) {
					if(regExp.test(this.value)){
						//소수점 첫째자리는 0 혹은 5로 제한
						var split = this.value.split('.');
						if(split.length > 1){
							if(split[1] == '0' || split[1] == '5' || split[1] == '')
								prev = this.value;
							else
								this.value = prev;
						}else{
				...
				//hh:mm 마스킹 처리
				$("#js-ship-mobile-proc-reg-insp-time").mask('99:99');
				
				//자리수 제한
				$('#js-ship-mobile-proc-reg-st').attr('maxlength', 7);
				$('#js-ship-mobile-proc-reg-dt, #js-ship-mobile-proc-reg-wt').attr('maxlength', 7);
					...
					success: function(response, status, jqueryXHR) {
						...
						if (typeof response.message !== 'undefined') {
							...
							if (message === 'OK') {
								...
								{
									$('#js-ship-mobile-proc-reg-weather-condition').val(response.rs[0].weatherCondition);
									$('#js-ship-mobile-proc-reg-dt').val(response.rs[0].dt);
									$('#js-ship-mobile-proc-reg-wt').val(response.rs[0].wt);
									$('#js-ship-mobile-proc-reg-wt').trigger('change');
									$('#js-ship-mobile-proc-reg-st').val(response.rs[0].st);
									$('#js-ship-mobile-proc-reg-dp').val(response.rs[0].dp);
									$('#js-ship-mobile-proc-reg-dp-display').val(response.rs[0].dp);
									$('#js-ship-mobile-proc-reg-rh').val(response.rs[0].rh);
									$('#js-ship-mobile-proc-reg-rh-display').val(response.rs[0].rh);
									$('#js-ship-mobile-proc-reg-insp-time').val(response.rs[0].inspTime);
									
									if(response.rs[0].amineYn != 'Y')
									{
										$('#js-ship-mobile-proc-reg-amine-div').click();
									}
									
									if(response.rs[0].sweatIcingYn != 'Y')
									{
										$('#js-ship-mobile-proc-reg-sweat-icing-div').click();
									}
								}
			...
			var retrieveShipWeatherCond = {
					success: function(response, status, jqueryXHR) {
						...
						if (typeof response.message !== 'undefined') {
							...
							if (message === 'OK') {
								$('#js-ship-mobile-proc-reg-weather-condition').find('option').remove();
								
								for(var i=0; i<response.rs.length; i++)
								{
									$('#js-ship-mobile-proc-reg-weather-condition').append("<option value='"+response.rs[i].code+"'>"+response.rs[i].codeNm+"</option>");
								}
								
								var cnt = response.rs.length;
								$('#js-ship-mobile-proc-reg-weather-condition').attr("title", cnt+" <spring:message code='TODO.KEY' text='건'/>");
							}else
							{
								// 서비스 요청 처리 오류 발생 메시지
								//console.log(message);
					...
					success: function(response, status, jqueryXHR) {
						...
						if (typeof response.message !== 'undefined') {
							...
							if (message === 'OK') {
								...
								if(response.rs.length > 0)
								{
									dp = response.rs[0].dewPoint;
									rh = response.rs[0].relHumidity;
								}
								
								$('#js-ship-mobile-proc-reg-dp').val(dp);
								$('#js-ship-mobile-proc-reg-dp-display').val(dp);
								$('#js-ship-mobile-proc-reg-rh').val(rh);
								$('#js-ship-mobile-proc-reg-rh-display').val(rh);
			...
			var retrieveShipMobileProcInfoRegStep5 = function(seqShipProcInfo){
				...
				if(!isDecimal(dt)){
					fn_alertInfo("A", '<spring:message code="TODO.KEY" text="잘못된 소수점 형식입니다."/>', function(){
						$('#js-ship-mobile-proc-reg-dt').focus();
					});
					return;
				}
				if(!isDecimal(wt)){
					fn_alertInfo("A", '<spring:message code="TODO.KEY" text="잘못된 소수점 형식입니다."/>', function(){
						$('#js-ship-mobile-proc-reg-wt').focus();
					});
					return;
				}
				if(!isDecimal(st)){
					fn_alertInfo("A", '<spring:message code="TODO.KEY" text="잘못된 소수점 형식입니다."/>', function(){
						$('#js-ship-mobile-proc-reg-st').focus();
					});
					return;
				}
				var timeRegExp = /^([1-9]|[01][0-9]|2[0-3]):([0-5][0-9])$/
				if(!app.utils.isEmpty(inspTime) && !timeRegExp.test(inspTime)){
					fn_alertInfo("A", '<spring:message code="TODO.KEY" text="잘못된 시간 형식입니다."/>', function(){
						$('#js-ship-mobile-proc-reg-insp-time').focus();
					});
					return;
				}
				
				if($('#js-ship-mobile-proc-reg-amine-div').find('.active').hasClass('toggle-on'))
					amineYn = 'Y';
				if($('#js-ship-mobile-proc-reg-sweat-icing-div').find('.active').hasClass('toggle-on'))
					sweatIcingYn = 'Y';
				
				$('#js-ship-mobile-proc-reg-amine-yn').val(amineYn);
				$('#js-ship-mobile-proc-reg-sweat-icing-yn').val(sweatIcingYn);
