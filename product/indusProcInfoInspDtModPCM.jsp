...
<div class="wp-dim-layer">
    ...
    <div id="js-indus-proc-info-insp-dt-mod-pcm-popup" class="wp-pop-layer wp-pop-w940 wp-box-sizing wp-car-popup">
    	<div class="wp-pop-title">
    		<h1><spring:message code="TODO.KEY" text="공정별 공정정보 등록-PCM"/></h1>
    		<button type="button" class="wp-btn-close-layer wp-spr-btn" id="js-indus-proc-info-insp-dt-mod-pcm-close"><span><spring:message code="TODO.KEY" text="닫기"/></span></button>
    	</div>
        <div class="wp-pop-conts">
			<!-- scroll Area -->
			<div class="wp-pop-cont-scrollable"  id="js-indus-proc-info-insp-dt-mod-pcm-proc-type-info">
			<input type="hidden" data-field="setFlag" value="" />
				...
				<div class="wp-clearfix">
					...
					<button type="button" id="js-indus-proc-info-insp-dt-mod-pcm-prod-lot-info-input-button" class="wp-btn small white bgray plus wp-mb5 wp-fr"><spring:message code="TODO.KEY" text="추가"/></button>
				</div>
				<div class="wp-table-st1" id="js-indus-proc-info-insp-dt-mod-pcm-prod-lot-info">
					<table>
						<caption></caption>
						<colgroup>
				...
				<div class="wp-table-st1 wp-issue-apply" id="js-indus-proc-info-insp-dt-mod-pcm-prod-info">
					<table>
				...
				<div class="wp-table-st1 wp-issue-apply" id="js-indus-proc-info-insp-dt-mod-pcm-coating-roll-info">
					<table>
				...
				<div class="wp-table-st1 wp-issue-apply" id="js-indus-proc-info-insp-dt-mod-pcm-diluent-info">
					<table>
						<caption><spring:message code="TODO.KEY" text="제품 정보"/></caption>
						<colgroup>
							<col style="width:15%">
				...
				<div class="wp-table-st1 wp-issue-apply" id="js-indus-proc-info-insp-dt-mod-pcm-maker-info">
					<table>
						...
						<tbody>
							<tr>
								...
								<td class="wp-al wp-vm">
									...
									<button type="button" class="wp-btn small gray cr3" id="js-indus-proc-info-insp-dt-mod-pcm-maker-info-input-button"><spring:message code="TODO.KEY" text="등록"/></button>
			...
			<div class="wp-btn-area">
				<button type="button" class="wp-btn blue1 cr3 wp-pl18 wp-pr18" id="js-indus-proc-info-insp-dt-mod-pcm-set-button"><spring:message code="TODO.KEY" text="확인"/></button>
			</div>
...
<script id="js-indus-proc-info-insp-dt-mod-pcm-prod-lot-template" type="text/x-handlebars-template">
	<tr class="wp-td-padding0" data-field="items-{{index}}" name="itemsList" dataIndex="{{index}}">
		...
		<td>
			<button type="button" id="js-indus-proc-info-insp-dt-mod-pcm-prod-lot-delete-button-{{index}}" dataIndex="{{index}}" name="js-indus-proc-info-insp-dt-mod-pcm-prod-lot-delete-button" class="wp-btn small white bgray trash"><span><spring:message code="TODO.KEY" text="삭제"/></span></button>
		</td>
...
<script id="js-indus-proc-info-insp-dt-mod-pcm-maker-template" type="text/x-handlebars-template">		
	{{#rows}}
		<li><span data-field="makerNmList">{{makerNm}}%</span><button type="button" class="wp-spr-btn wp-close-sel" name="js-indus-proc-info-insp-dt-mod-pcm-maker-delete-button"><span><spring:message code="TODO.KEY" text="닫기"/></span></button>
...
var indusProcInfoInspDtModPCM = (function() {
	...
	var initData = {
		init: function(){
			...
			// 적용 공정 attr
			$procType = $('#js-indus-proc-info-insp-dt-mod-pcm-proc-type-info');
			// 적용 제품/LOT No.정보 attr
			$prodLot =  $('#js-indus-proc-info-insp-dt-mod-pcm-prod-lot-info');
			// 제품 정보 attr
			$prod = $('#js-indus-proc-info-insp-dt-mod-pcm-prod-info');
			// 공정 Coating Roll 정보 attr
			$coatingRoll = $('#js-indus-proc-info-insp-dt-mod-pcm-coating-roll-info');
			// 희석 조건 attr
			$diluent = $('#js-indus-proc-info-insp-dt-mod-pcm-diluent-info');
			// 도료 Supplier 점유율 attr
			$maker = $('#js-indus-proc-info-insp-dt-mod-pcm-maker-info');
		},
	...
	var pageQueryEvent = {
		...
		makerAppend: function(data){ // 도료 Supplier 점유율
			...
			// 닫기 버튼 Event 
			$('button[name=js-indus-proc-info-insp-dt-mod-pcm-maker-delete-button]').off().on('click', function(){
				$(this).parent().remove();
		...
		prodLotEvent: function(){ // 적용 제품/LOT No.정보
			// 제품코드 조회 버튼 클릭
			$("button[name=js-indus-proc-info-insp-dt-mod-pcm-prod-list-button]").off().on("click", function(){
				fn_openPop($(this));
				prodItemsInfoRowIndex = $(this).attr("dataIndex");
				initRetrieveProdItemsInfoList("js-indus-proc-info-pcm-mod-popup");   
			});
			
			//삭제 버튼 클릭
			$("button[name=js-indus-proc-info-insp-dt-mod-pcm-prod-lot-delete-button]").off().on("click", function(e) {
		...
		setData: function(){ // 확인 버튼 클릭
			...
			indusProcInfoInspModPCM.ifFn.pageInit();
			
			$("#js-indus-proc-info-insp-dt-mod-pcm-close").click();
		},
		chkProcType: function(){
			// 적용 공정 등록 된 데이터 확인
			if(indusProcInfoInspModPCM.chkPorcId($procType.find('select[data-field="procType"]').val())){
				app.message.alert({
					  title             : '<spring:message code="TODO.KEY" text="알림"/>'
		...
		init: function() {
			// 도료 Supplier 점유율
			var source = $('#js-indus-proc-info-insp-dt-mod-pcm-maker-template').html();
			makerListTemplate = Handlebars.compile(source);
			// 도료 Supplier 점유율(%) 등록
			$('#js-indus-proc-info-insp-dt-mod-pcm-maker-info-input-button').off().on('click', this.makerReg);
			
			// 적용 제품/LOT No.정보
			var source = $('#js-indus-proc-info-insp-dt-mod-pcm-prod-lot-template').html();
			prodLotListTemplate = Handlebars.compile(source);
			// 적용 제품/LOT No.정보 등록
			$('#js-indus-proc-info-insp-dt-mod-pcm-prod-lot-info-input-button').off().on('click', this.prodLotReg);
			// 확인 버튼
			$('#js-indus-proc-info-insp-dt-mod-pcm-set-button').off().on('click', this.setData);
