...
<%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c" %>
	...
	<div class="wp-dim-layer">
   		...
   		<div id="js-indus-proc-info-insp-reg-alwheel-popup" class="wp-pop-layer wp-pop-w940 wp-box-sizing wp-car-popup">
	    	<div class="wp-pop-title">
	    		<h1><spring:message code="TODO.KEY" text="거래선 공정정보 등록-AlWheel"/></h1>
	    		<button type="button" id="js-indus-proc-info-insp-reg-alwheel-close" class="wp-btn-close-layer wp-spr-btn"><span>닫기</span></button>
	    	</div>
	        <div class="wp-pop-conts">
				...
				<div class="wp-pop-cont-scrollable">
					...
					<div class="wp-table-st1 wp-issue-apply" id="js-indus-proc-info-insp-reg-alwheel-basic-info">
						<table>
					...
					<div class="wp-table-st1 wp-issue-apply" id="js-indus-proc-info-insp-reg-alwheel-line-info">
						<table>
					...
					<div class="wp-table-st1 wp-issue-apply" id="js-indus-proc-info-insp-reg-alwheel-material-info">
						<table>
					...
					<div class="wp-table-st1 wp-issue-apply" id="js-indus-proc-info-insp-reg-alwheel-environment-info">
						<table>
					...
					<div class="wp-table-st1 wp-issue-apply" id="js-indus-proc-info-insp-reg-alwheel-rate-info">
						<table>
					...
					<div class="wp-clearfix">
						...
						<button type="button" id="js-indus-proc-info-insp-dt-reg-alwheel-button" data-link="js-indus-proc-info-insp-dt-reg-alwheel-popup" data-depth="2" class="wp-btn small white bgray plus wp-mb5 wp-fr"><spring:message code="TODO.KEY" text="추가"/></button>
					</div>
					<div class="wp-wrap-dl-paint-check02" id="js-indus-proc-info-insp-dt-reg-alwheel">
					</div>
					<!-- //공정별 공정 정보 -->
					<!-- // scroll Area -->
				</div>
				...
				<div class="wp-btn-area">
					<button type="button" id="js-indus-proc-info-reg-alwheel-popup-save-button" class="wp-btn big blue1 cr3 wp-pl18 wp-pr18">저장</button>
					<input type="hidden"  id="js-indus-proc-info-reg-alwheel-pop-docNo"     value="">
					<input type="hidden"  id="js-indus-proc-info-reg-alwheel-pop-orgDocNo"  value="">
					<input type="hidden"  id="js-indus-proc-info-reg-alwheel-pop-item-type" value="">
				</div>
...
<!-- 공정별 공정 정보 - 스크립트-->
<script id="js-indus-proc-info-insp-dt-reg-alwheel-template" type="text/x-handlebars-template">
{{#rows}}
        ...
        <a class="wp-list-paint-check02 wp-pointer">
            <div class="wp-clearfix">
                ...
                <div class="wp-rp-bttn">
                    <button type="button" class="wp-btn small white bgray mody" data-field="itemMod" data-link="js-indus-proc-info-insp-dt-reg-alwheel-popup" data-depth="2" dataIndex="{{dataIndex}}" procId="{{procId}}"><span><spring:message code="TODO.KEY" text="수정"/></span></button>
    ...
    <dd class="wp-list-paint-check02-view" style="display: none;">
        ...
        <div class="wp-list-paint-check02 on">
            <div class="wp-clearfix">
                <strong class="wp-tit bold"><spring:message code="TODO.KEY" text="적용 공정 : "/>{{procNm}}</strong>
                <div class="wp-rp-bttn">
                    <button type="button" class="wp-btn small white bgray mody" data-field="itemModSub" data-link="js-indus-proc-info-insp-dt-reg-alwheel-popup" data-depth="2" dataIndex="{{dataIndex}}" procId="{{procId}}"><span><spring:message code="TODO.KEY" text="수정"/></span></button>
...
var indusProcInfoInspRegAlWheel = (function() {
	// item code
	...
	var initData = {
		init: function(){
			...
			// 공정별 공정 정보
			$('#js-indus-proc-info-insp-dt-reg-alwheel').empty();
			
			// 기본정보 attr
			$basicInfo = $('#js-indus-proc-info-insp-reg-alwheel-basic-info');
			// 라인정보 attr
			$line = $('#js-indus-proc-info-insp-reg-alwheel-line-info');
			//소지 정보 attr
			$material = $('#js-indus-proc-info-insp-reg-alwheel-material-info');
			// 환경 조건 및 특이사항 attr
			$environment = $('#js-indus-proc-info-insp-reg-alwheel-environment-info');
			//도장 생산성 정보 attr
			$rate = $('#js-indus-proc-info-insp-reg-alwheel-rate-info');
			//공정별 공정 정보 attr(적용 제품/LOT No.정보)
			$item = $('#js-indus-proc-info-insp-dt-reg-alwheel');
		},
		...
		setInfo: function(docNo, itemType){
			...
			$("#js-indus-proc-info-reg-alwheel-pop-item-type").val(itemType);

			// 공정별 공정 정보
			var source = $('#js-indus-proc-info-insp-dt-reg-alwheel-template').html();
	...
	var pageQueryEvent = {
		init: function(){
			...
			$("#js-indus-proc-info-insp-dt-reg-alwheel-button").off().on('click', function(e){
				fn_openPop($(this));
				indusProcInfoInspDtRegAlWheel.init(
			...
			$basicInfo.find('button[data-field="empSearchButton"]').off().on('click', function(){
				initIndustryEmpRetrievePopup('js-indus-proc-info-reg-alwheel-popup');
			});
			
			// 저장
			$("#js-indus-proc-info-reg-alwheel-popup-save-button").off().on('click', this.saveData);
		},
	...
	var ifFn = {
		...
		pageSetData: function() {
			...
			$('#js-indus-proc-info-insp-dt-reg-alwheel').empty().append(result);
			...
			$item.find('button[data-field="itemMod"]').off().on("click", function() {
				...
				for(var j=0; j<procList.length; j++){
					if(procList[j].dtProcId.id == procId){
						indusProcInfoInspDtRegAlWheel.ifFn.setModData(procList[j].dtProcId, procList[j].dtProcInfoDt, procList[j].dtShare, procList[j].dtItem);
					}
				}
		...
		pageInit: function() {
			var dtProcId = indusProcInfoInspDtRegAlWheel.ifFn.getProcId();
			var dtProcInfoDt = indusProcInfoInspDtRegAlWheel.ifFn.getProcInfoDt();
			var dtShare = indusProcInfoInspDtRegAlWheel.ifFn.getShare();
			var dtItem = indusProcInfoInspDtRegAlWheel.ifFn.getItem();
			// 공정별 공정 정보 attr
		...
		success: function(response, status, jqueryXHR) {
			...
			if (typeof response.message !== 'undefined') {
				...
				if (message === 'OK') {
					...
					$("#js-indus-proc-info-insp-reg-alwheel-close").click();
