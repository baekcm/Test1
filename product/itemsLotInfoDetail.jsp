...
<%@ page info="[2017.08.23] 산업용 제품 출고 이력 상세" %>
<%@ taglib uri="http://www.springframework.org/tags" prefix="spring" %>
<%@ taglib uri="http://tiles.apache.org/tags-tiles" prefix="tiles" %>
<%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c" %>

<tiles:insertDefinition name="app.industry.popup">
	<%-- 본문 페이지 --%>
			...
			<div class="wp-pop-conts wp-mr20">
				...
				<div class="wp-table-st1">
					<table>
						...
						<thead>
							<th class="wp-al"><spring:message code="TODO.KEY" text="생산공장"/>: <span class="wp-mr20" id="js-items-lot-info-detail-feedback-prodPlant"></span>
								<spring:message code="TODO.KEY" text="생산일자"/>: <span class="wp-mr20" id="js-items-lot-info-detail-feedback-prodDate"></span>
								<spring:message code="TODO.KEY" text="제품번호"/>: <span class="wp-mr20" id="js-items-lot-info-detail-feedback-lotNo"></span>
								<spring:message code="TODO.KEY" text="배합번호"/>: <span class="wp-mr20" id="js-items-lot-info-detail-feedback-formula"></span>
								<spring:message code="TODO.KEY" text="진행상태"/>: <span id="js-items-lot-info-detail-feedback-feedbackStatus"></span>
							</th>
						</thead>
						<tbody>
							<tr>
				...
				<div class="wp-title-btn-area wp-mb10">
					<h2 class="wp-cont-title"><spring:message code="TODO.KEY" text="품질 이슈"/></h2>
		...
		<script type="text/javascript">
			...
			var initHandlebarsTemplateDetail = function(sParam) {
				...
				$('#js-quality-issue-q-voc-link').off().on('click', function(){
					...
					{						
						messageParam.title = "알림";
						messageParam.message = "IE 브라우저만 사용 가능합니다";
			...
			var retrieveOrderInfoDetail = {
				success : function(response, status, jqueryXHR) {
					...
					if (typeof response.message !== 'undefined') {
						...
						if (message === 'OK') {
							...
							$('#js-items-lot-info-detail-psize-unit').text($.number(parseInt(response.rs.orderInfo[0].psize)) + " / " + response.rs.orderInfo[0].unit);
							...
							$('#js-items-lot-info-detail-repProdDept').text(response.rs.prodInfo[0].repProdPlant);
							...
							//$('#js-items-lot-info-detail-stockVqty').append(response.rs.prodInfo[0].stockVqtyUnit);
							...
							if(response.rs.feedbackInfo != null){
								if(response.rs.feedbackInfo.length > 0){
									//배합Feedback
									$('#js-items-lot-info-detail-feedback-prodPlant').text(response.rs.feedbackInfo[0].prodPlant);
									$('#js-items-lot-info-detail-feedback-prodDate').text(response.rs.feedbackInfo[0].prodDate);
									$('#js-items-lot-info-detail-feedback-lotNo').text(response.rs.feedbackInfo[0].lotNo);
									$('#js-items-lot-info-detail-feedback-formula').text(response.rs.feedbackInfo[0].formula);
									$('#js-items-lot-info-detail-feedback-feedbackStatus').text(response.rs.feedbackInfo[0].feedbackStatus);
									$('#js-items-lot-info-detail-feedback-reqContent').text(response.rs.feedbackInfo[0].reqContent);
								}
							}
				...
				request : function(cust, sProdPlant, sItems, sLotNo, sOutNo, sOutSeq, sOutDate, sOutVqty, sFormula, sRepOrderNo, sOrderId) {
					// ------------------------------------
					// ajax request
					// ------------------------------------
			...
			var pageBodyHandler = {
				init: function() {
					...
					retrieveOrderInfoDetail.request('${param.cust}', '${param.sProdPlant}', '${param.sItems}', '${param.sLotNo}', '${param.sOutNo}'
							, '${param.sOutSeq}', '${param.sOutDate}', '${param.sOutVqty}', '${param.sFormula}'
							, '${param.sRepOrderNo}', '${param.sOrderId}');
