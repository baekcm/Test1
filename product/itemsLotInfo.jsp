		...
		<section class="wp-stat">
			<div class="wp-cont wp-car-sub">
				...
				<header class="wp-contents-title">
					...
					<div class="wp-location"><span><spring:message code="TODO.KEY" text="제품정보"/></span><span class="wp-fc-54"><spring:message code="TODO.KEY" text="제품 출고 이력 정보"/></span><span class="wp-fc-54"><spring:message code="TODO.KEY" text="LOT별 생산/검사/출고 이력 정보"/></span></div>
				</header>
				<div class="wp-table-width">
					...
					<div class="wp-ui-select wp-wd190">
						<select id="js-items-lot-info-search-cust-combo" title="<spring:message code='TODO.KEY' text='거래선 선택'/>">
						</select>
					</div>
	...
	<tiles:putAttribute name="page-level-js">
		...
		<script id="js-template-items-lot-info-list" type="text/x-handlebars-template">		
			<div class="wp-timeline">
				...
				{{#rows}}
					...
					{{/isYearYn}}
					<div class="wp-box-section {{setClassGubun formulaChangeYn lotChangeYn expiredYn}}" style="cursor:default">
						<h3 class="wp-box-title">
							<!--<span>거래선: {{custNm}}</span>--!>
							<span>제품명: {{itemsNm}}</span>
							<span>LOT No.: {{lotNo}}</span>
							<span>Formula No.: {{formula}}</span>
							<span>저장기간: {{shelfLife}}</span>
							<span>만기일자: {{expiryDate}}</span>
						</h3>
						...
						<div class="wp-box-etc">
							...
							<span>제품코드: {{items}}</span>
							<span>입고일자: {{lastInDate}}</span>
							<span>입고량({{unit}}): {{setComma prodInVqty}}</span>
							<span>출고량({{unit}}): {{setComma outVqty}}</span>
						</div>
						<div class="wp-btn-section">
							<button type="button" class="wp-btn wp-btn-pop small white bgray" id="js-items-lot-info-detail-reqist-{{outNo}}-{{outSeq}}" name="js-items-lot-info-detail-popup-button-all" data-link="js-items-lot-info-detail-popup" data-depth="1" value="{{cust}}" data-items = "{{items}}" data-lotNo = "{{lotNo}}" data-outNo = "{{outNo}}" data-outSeq = "{{outSeq}}" data-prodPlant = "{{prodPlant}}" data-formula = "{{formula}}" data-outDate="{{outDate}}" data-outVqty="{{outVqty}}" data-repOrderNo="{{repOrderNo}}" data-orderId="{{orderId}}">자세히보기</button>							
						</div>
					</div>
				{{/rows}}
			</div>
		...
		<script type="text/javascript">
			...
			$(document).ready(function() {
				...
				var beforeDate =  fn_getBeforeDate(date, 0, 0, 2);
				...
				$('#js-items-lot-info-search-cust-combo').off().on("change", function(){
					retrieveProdLotHist.request();
				});
			});
			...
			var retrieveIndCustStdInfoAll = function(){
				...
				//거래선 combo 조회
				retrieveIndustCustData.request();
				retrieveProdLotHist.request();
				...
				success: function(response, status, jqueryXHR) {
					...
					if (typeof response.message !== 'undefined') {
						...
						if (message === 'OK') {							
							//셀렉트 항목 적용
							var comboCnt = 0;
							$('#js-items-lot-info-search-cust-combo').find('option').remove();
							for(var i=0; i<response.rs.length; i++) 	{
								$('#js-items-lot-info-search-cust-combo').append("<option value='"+response.rs[i].code+"'>"+response.rs[i].codeNm+"</option>");
								comboCnt++;									
							}
							$('#js-items-lot-info-search-cust-combo').attr("title", comboCnt+" <spring:message code='TODO.KEY' text='건'/>");
						} else 	{
							// 서비스 요청 처리 오류 발생 메시지
			...
			var winOpenHandler = {
				createProductHistoryDetail : function(params) {
					...
					app.utils.window.open({ 
						url: '/industry/product/itemsLotInfoDetail.do', options: options, params: params 
					}); 
			...
			var retrieveProdLotHist = {
						...
						if (message === 'OK') {
							...
							$("button[name=js-items-lot-info-detail-popup-button-all]").off().on('click', function() {								
								...
								var params = { 
									...
									sLotNo: $(this).attr("data-lotNo"),			//LOT No
									sOutNo: $(this).attr("data-outNo"),			//출고번호
									sOutSeq: $(this).attr("data-outSeq"),		//출고순번
									sOutDate: $(this).attr("data-outDate"),		
									sOutVqty: $(this).attr("data-outVqty"),		
									sFormula: $(this).attr("data-formula"),		//배합번호
									sRepOrderNo: $(this).attr("data-repOrderNo"),
									sOrderId: $(this).attr("data-orderId")	
								}; 
				...
				request : function(gubun) {
					...
					//헤더 거래선 정보 가져오면 주석해제 할것
					var cust 	  	= $('#js-items-lot-info-search-cust-combo').val();
					//$('#js-industry-header-item-type').val();				
					...
					{						
						app.message.alert({
							title: '알림', message: '<spring:message code="TODO.KEY" text="거래선은 필수입력 항목입니다."/>', confirmButtonText: '확인',
							callback: {
								confirm: function() {
